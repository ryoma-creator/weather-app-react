...
export const FilterTabs = ({ onFilterChange }) => {
  ...
  const handleFilterChange = (value) => {
    if (activeFilterTab === "Category") {
      ...
      onFilterChange({ category: value, priority: priorityFilter });
    } else {
      setPriorityFilter(value);
      onFilterChange({ category: categoryFilter, priority: value });
    }
  };
      ...
      <div className="space-y-4">
        <ul className="flex space-x-4">
          {filterTabs.map((filterTab) => (
            <li
              key={filterTab.name}
              className={`cursor-pointer flex px-4 py-2 rounded-full items-center gap-2 text-slate-500 shadow-[-5px_-5px_10px_rgba(255,_255,_255,_0.8),_5px_5px_10px_rgba(0,_0,_0,_0.25)] hover:shadow-[-1px_-1px_5px_rgba(255,_255,_255,_0.6),_1px_1px_5px_rgba(0,_0,_0,_0.3),inset_-2px_-2px_5px_rgba(255,_255,_255,_1),inset_2px_2px_4px_rgba(0,_0,_0,_0.3)] hover:text-violet-500 ${activeFilterTab === filterTab.name ? "bg-white text-black !important" : "text-gray-500"}`}
        ...
        <div className="flex items-center space-x-6 py-4">
          ...
          {filterTabs
            ...
            .filters.map((filter) => (
              <motion.div
                key={filter}
                className={`rounded-full px-4 py-2 flex items-center gap-2 text-slate-500 shadow-[-5px_-5px_10px_rgba(255,_255,_255,_0.8),_5px_5px_10px_rgba(0,_0,_0,_0.25)] cursor-pointer ${
